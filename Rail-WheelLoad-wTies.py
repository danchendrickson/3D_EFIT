...
TiesLocation=np.zeros((gl1,gw1,gh1))
...
if FFunction == 1:
    ...
    #starting at just so the second wheel is off the patern would be 1.35.  
    #this run has it at .75, so between 2 ties
    WheelStartPoint = int(0.75 / gs)
...
if Ties > 1:
    TieSpacing = length1 / (Ties)
    TieGap = int(TieSpacing / gs)
    
    TieWidth = int(0.15 / gs)  #15cm tie
    
    for i in range(Ties):
        TiesLocation[TieGap*i+1:TieGap*i+TieWidth-1,:,2]=(1/(i+1))
        TiesLocation[TieGap*i:TieGap*i+TieWidth,:,3]=(0.5/(i+1))
        TiesLocation[TieGap*i,:,2]=(0.5/(i+1))
        TiesLocation[TieGap*i+TieWidth,:,2]=(0.5/(i+1))
        
    TiesForce = WheelLoad / np.sum(TiesLocation)

if Ties == 1:
    TieCenter = int(0.75 / gs)
    TieWidth = int(0.15 / gs)  #15cm tie
    HalfTie = int(TieWidth / 2)
    
    TiesLocation[TieCenter - HalfTie + 1: TieCenter + HalfTie - 1,:,2]=1
    TiesLocation[TieCenter - HalfTie: TieCenter + HalfTie,:,3]=0.5
    TiesLocation[TieCenter - HalfTie,:,2]=0.5
    TiesLocation[TieCenter + HalfTie,:,2]=0.5
...
mpi_comm.Scatterv([TiesLocation[:,:,:],split,offset,MPI.DOUBLE], tiesloc)
...
tiesloc=distBox(tiesloc,myid,gl1,gw1,gh1,npx,nprocs,mpi_comm)        
